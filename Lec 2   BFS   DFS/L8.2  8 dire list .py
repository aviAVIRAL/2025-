...
# # -----------for 8 direction 
# del_row = [-1, -1, -1, 0, 1, 1, 1, 0]
# del_col = [-1, 0, 1, 1, 1, 0, -1, -1]
# # ---------------------------
    while q:
        row, col = q.popleft()
# -------  neighbor travel -------------# ---------------
        
        # for k in range(8):  # for 8 direction 
        for k in range(4):    # for 4 direction only
            new_row = row + del_row[k]
            new_col = col + del_col[k]
